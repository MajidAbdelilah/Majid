...
void createShaderStorageBuffers(State *state) {
  ...
  createBuffer(state, bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer,
               &stagingBufferMemory);
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    createBuffer(state, bufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &state->shaderStorageBuffers[i], &state->shaderStorageBuffersMemory[i]);
...
void createGraphicsPipeline(State *state) {
  ...
  VkPipelineLayoutCreateInfo pipelineLayoutInfo = {0};
  pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
  VkDescriptorSetLayout descriptorLayouts[] = {state->descriptorSetLayout};
  pipelineLayoutInfo.setLayoutCount = sizeof(descriptorLayouts) / sizeof(VkDescriptorSetLayout);
...
void createBlendPipeline(State *state) {
  ...
  if (vkCreatePipelineLayout(state->device, &pipelineLayoutInfo, NULL, &state->blendPipelineLayout) != VK_SUCCESS) {
    fprintf(stderr, "failed to create pipeline layout!\n");
  } else {
    printf("pipeline layout secessfully created\n");
  }
  ...
  if (vkCreateGraphicsPipelines(state->device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &state->blendPipeline) != VK_SUCCESS) {
    fprintf(stderr, "failed to create graphics pipeline!\n");
  } else {
    printf("pipeline was secessfully created\n");
  }
...
void recordCommandBuffer(State *state, VkCommandBuffer commandBuffer, uint32_t imageIndex) {
  ...
  for (unsigned long i = 0; i < state->models_count; i++) {
    for (uint32_t j = 0; j < state->models[i].meshCount; j++) {
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, state->pipelineLayout, 0, 1, descriptorsets, 0, NULL);

      VkDeviceSize offsets[] = {0};
      vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
      vkCmdBindIndexBuffer(commandBuffer, state->models[i].meshes[j].vertexIndexUniformBuffer, sizeof(Vertex) * state->models[i].meshes[j].verticesCount,
...
void printf_matrix(struct mat4 matrix) {
...
void updateUniformBuffer(State *state, Camera3D_r camera, uint32_t currentImage) {
  ...
  for (uint32_t i = 0; i < state->models_count; i++) {
    ...
    for (uint32_t j = 0; j < state->models[i].meshCount; j++) {
      ...
      psmat4_translation(&position, &position, (struct vec3[]){{0.0, 0.0, 0.0}});
      ...
      psmat4_scale(&scaling, &scaling, (struct vec3[]){{2.00f, 2.00f, 2.00f}});
      ...
      psmat4_perspective((&state->models[i].meshes[j].ubo.proj), to_radians(75.0f), (float)state->swapChainExtent.width / (float)state->swapChainExtent.height,
                         0.1f, 1000.0f);
      ...
      if (state->models[i].meshes[j].updateUbo) {
        ...
        unsigned int bufferSize = sizeof(Vertex) * state->models[i].meshes[j].verticesCount + sizeof(uint32_t) * state->models[i].meshes[j].indicesCount;
        ...
        vkCmdUpdateBuffer(commandBuffer, state->models[i].meshes[j].vertexIndexUniformBuffer, (size) + (alinedSize * (currentImage)),
...
void createFragShaderUniform(State *state, Mesh *mesh) {
  ...
  // fsuBuffersMemory = malloc(sizeof(VkDeviceMemory) * MAX_FRAMES_IN_FLIGHT);
  // model->uniformBuffersMapped = malloc(sizeof(void *) * MAX_FRAMES_IN_FLIGHT);

  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    createBuffer(state, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                 &mesh->fsuBuffers[i], &mesh->fsuBuffersMemory[i]);
...
void createVertexIndexUniformBuffer(State *state, Mesh *mesh) {
  ...
  VkDeviceSize bufferSize = sizeof(Vertex) * mesh->verticesCount + sizeof(uint32_t) * mesh->indicesCount;
  ...
  createBuffer(state, bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &mesh->vertexIndexUniformBuffer, &mesh->vertexIndexUniformBufferMemory);
...
void createDescriptorPool(State *state) {
  VkDescriptorPoolSize poolSizes[30000] = {0};
  for (uint32_t i = 0; i < (sizeof(poolSizes) / sizeof(VkDescriptorPoolSize)) / 3; i++) {

    poolSizes[i * 3 + 0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    poolSizes[i * 3 + 0].descriptorCount = (MAX_FRAMES_IN_FLIGHT);

    poolSizes[i * 3 + 1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    poolSizes[i * 3 + 1].descriptorCount = (MAX_FRAMES_IN_FLIGHT);

    poolSizes[i * 3 + 2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[i * 3 + 2].descriptorCount = (MAX_FRAMES_IN_FLIGHT);
  }
...
void createDescriptorSets(State *state, VkDescriptorSet *descriptorSet, VkImageView textureImageView, VkSampler textureSampler,
                          VkBuffer *vertexIndexUniformBuffers, unsigned int uboOffset, VkBuffer *fsu) {
...
void createImage(State *state, uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling,
...
void createColorResources(State *state) {
  ...
  createImage(state, state->swapChainExtent.width, state->swapChainExtent.height, 1, state->msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &state->colorImage,
              &state->colorImageMemory);
  state->colorImageView = createImageView(state, state->colorImage, 1, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT);
}

void createTextureImage(State *state, const char *path, uint32_t *mipLevels, VkImage *textureImage, VkDeviceMemory *textureImageMemory) {
  M_image image = M_load_image(path);

  *mipLevels = floorf(log2f(um_max(image.texWidth, image.texHeight))) + 1;
...
void createTextureSampler(State *state, uint32_t *mipLevels, VkSampler *textureSampler) {
  ...
  if (vkCreateSampler(state->device, &samplerInfo, NULL, textureSampler) != VK_SUCCESS) {
...
static inline um_vec2 ufbx_to_um_vec2(ufbx_vec2 v) { return um_v2((float)v.x, (float)v.y); }
static inline um_vec3 ufbx_to_um_vec3(ufbx_vec3 v) { return um_v3((float)v.x, (float)v.y, (float)v.z); }
static inline um_quat ufbx_to_um_quat(ufbx_quat v) { return um_quat_xyzw((float)v.x, (float)v.y, (float)v.z, (float)v.w); }
...
void parse_model(State *state, Majid_model *model) {
  ...
  for (unsigned long ni = 0; ni < model->scene->nodes.count; ni++) {
    ...
    if (node->mesh) {
      ...
      for (size_t pi = 0; pi < mesh->materials.count; pi++) {
        ufbx_mesh_part *mesh_mat = &mesh->materials.data[pi];
        if (mesh_mat->num_triangles == 0)
          continue;
        ...
        */
        ...
        for (uint32_t j = 0; j < num_vertices; j++) {
          ...
          */
          state->models[state->models_count].meshes[pi].vertices[j].pos = (struct vec3){vertices[j].position.x, vertices[j].position.y, vertices[j].position.z};
          state->models[state->models_count].meshes[pi].vertices[j].texCoord = (struct vec2){vertices[j].uv.x, -vertices[j].uv.y};
        ...
        optimize_vertex_index_buffers_for_GPU(
            state->models[state->models_count].meshes[pi].vertices, state->models[state->models_count].meshes[pi].verticesCount,
            state->models[state->models_count].meshes[pi].indices, state->models[state->models_count].meshes[pi].indicesCount);
        ...
        if (mesh_mat->textures.count > 0) {
          ...
          for (uint32_t fri = 0; fri < MAX_FRAMES_IN_FLIGHT; fri++) {
            ...
            state->models[state->models_count].meshes[pi].fsu.transparency_color =
                (struct vec4){1.0f, // mesh_mat->material->fbx.transparency_color.value_vec4.x * mesh_mat->material->fbx.transparency_factor.value_vec4.x,
          ...
          createTextureImage(state, name, &state->models[state->models_count].meshes[pi].mipLevels, &state->models[state->models_count].meshes[pi].textureImage,
                             &state->models[state->models_count].meshes[pi].textureImageMemory);

          createTextureImageView(state, &state->models[state->models_count].meshes[pi].mipLevels, &state->models[state->models_count].meshes[pi].textureImage,
                                 &state->models[state->models_count].meshes[pi].textureImageView);

          createTextureSampler(state, &state->models[state->models_count].meshes[pi].mipLevels, &state->models[state->models_count].meshes[pi].textureSampler);

          unsigned int bufferSize = sizeof(Vertex) * state->models[state->models_count].meshes[pi].verticesCount +
          ...
          createDescriptorSets(state, state->models[state->models_count].meshes[pi].descriptorSets,
                               state->models[state->models_count].meshes[pi].textureImageView, state->models[state->models_count].meshes[pi].textureSampler,
                               &state->models[state->models_count].meshes[pi].vertexIndexUniformBuffer, size,
          ...
          printf("transparency_color: x=%f, y=%f, z=%f, w=%f\ntransparency_factor x= %f, y= %f, z= %f, w= %f\n",
                 mesh_mat->material->fbx.transparency_color.value_vec4.x, mesh_mat->material->fbx.transparency_color.value_vec4.y,
                 mesh_mat->material->fbx.transparency_color.value_vec4.z, mesh_mat->material->fbx.transparency_color.value_vec4.w,
                 mesh_mat->material->fbx.transparency_factor.value_vec4.x, mesh_mat->material->fbx.transparency_factor.value_vec4.y,
                 mesh_mat->material->fbx.transparency_factor.value_vec4.z, mesh_mat->material->fbx.transparency_factor.value_vec4.w);

          state->models[state->models_count].meshes[pi].fsu.is_no_texture = false;
          state->models[state->models_count].meshes[pi].fsu.transparency_color = (struct vec4){
              1.0f, 1.0f, 1.0f, 1.0f
          ...
          createTextureImage(state, "white.jpg", &state->models[state->models_count].meshes[pi].mipLevels,
                             &state->models[state->models_count].meshes[pi].textureImage, &state->models[state->models_count].meshes[pi].textureImageMemory);
...
void init_vulkan(State *state) {
  ...
  for (unsigned int i = 0; i < state->models_count; i++) {
    for (unsigned int j = 0; j < state->models[i].meshCount; j++) {
      ...
      optimize_vertex_index_buffers_for_GPU(state->models[i].meshes[j].vertices, state->models[i].meshes[j].verticesCount, state->models[i].meshes[j].indices,
...
void renderer_loop() {
  while (!glfwWindowShouldClose(state.window)) {
    ...
    glfwPollEvents();
    drawFrame(&state);
    processCameraInput(&state.camera, state.window);
    // usleep(1000000 / 60);
